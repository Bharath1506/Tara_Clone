const url = "https://ai.talentspotify.com/api/objectives/getCompanyObjectives?userId=68e240b0d9876d59139672d6&companyId=6396f7d703546500086f0200&type=me&empId=68e240b0d9876d59139672d6";
const key = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..C59lyahish42UR6lh1jD2_-gojFVAUo6rm2SHC31h1Q";

async function testOkr() {
    try {
        const res = await fetch(url, { headers: { 'Authorization': `Bearer ${key}` }});
        const data = await res.json();
        console.log("OKR Data:", JSON.stringify(data).substring(0, 200));
    } catch (e) {
        console.error("OKR Error:", e);
    }
}
testOkr();
